 ...
 *                      --base=/<repository>/ so the hashed asset URLs carry the
	...
	return {
		...
		configureServer(server)
		{
			server.middlewares.use('/plan', (req, res, next) =>
			{
				...
				// Never cached. The plan is regenerated by tools/build.py while
				// the server is running, and this response carries no ETag and
				// no Last-Modified -- so without an explicit directive a browser
				// falls back to heuristic caching and is entitled to keep
				// serving a design.json that has since been rebuilt. Which it
				// does, and it looks exactly like the extractor having no
				// effect.
